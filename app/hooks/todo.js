...
export function useTodo() {
    ...
    useEffect(() => {
        // Fetch  a user profile if there is a profile then its TodoAccounts
            ...
            if (program && publicKey && !transactionPending) {
                try {
                    ...
                    if (profileAccount) {
                        ...
                        console.log("NOT_YET_INITIALIZED")
    ...
    const markTodo = async (todoIdx, todoPda) => {
        if (program && publicKey) {
            try {
                setTransactionPending(true)
                setLoading(true)
    ...
    const removeTodo = async (todoIdx, todoPda) => {
        if (program && publicKey) {
            try {
                setTransactionPending(true)
                setLoading(true)
